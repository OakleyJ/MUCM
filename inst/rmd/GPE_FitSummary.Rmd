...
library(MUCM)
#load fit data
load("D:/Work/R/MUCM/TempFit.RData")
...
```{r, echo = FALSE, results='asis'}
yr <- t(apply(fit$training.outputs, 2, range))
colnames(yr) <- c("minimum", "maximum")
yr <- signif(yr, 3)
knitr::kable(yr, padding = 0)
```
...
if (isTRUE(all.equal(corMatern2.5, fit$cor.function))) {
    cor.funct <- "Matern 2.5 which is given by the following equation:" 
...
\\[ \\Big[ \\frac{(x_{1}-x'_{1})^2}{\\delta_1^2} +...+ \\frac{(x_{p}-x'_{p})^2}{\\delta_p^2}  \\Big]^{0.5}.\\]")
    ...
    cor.funct <- "Gaussian which is given by the following equation:" 
...
+ Correlation function: `r paste(cor.funct, equation)`
...
Package_Name <- c("MUCM", Package_Name)
